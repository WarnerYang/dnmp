...
&&  curl -L  https://github.com/alanxz/rabbitmq-c/releases/download/v0.7.1/rabbitmq-c-0.7.1.tar.gz | tar xvz -C   /usr/src/rabbitmq-c --strip 1 \

&&   cd /usr/src/rabbitmq-c &&  ./configure --prefix=/usr/local/rabbitmq-c && make && make install
&&   mkdir -p /usr/src/php/ext/amqp
&&   curl -l https://pecl.php.net/get/amqp-1.9.3.tgz | tar xvz -C /usr/src/php/ext/amqp --strip 1
&&   echo 'amqp' >> /usr/src/php-available-exts &&  cp /usr/src/rabbitmq-c/librabbitmq/amqp_ssl_socket.h /usr/src/php/ext/amqp/
&&   docker-php-ext-configure amqp --with-librabbitmq-dir=/usr/local/rabbitmq-c  &&  docker-php-ext-install amqp
&&   php -m | grep amqp
EXPOSE 9000
